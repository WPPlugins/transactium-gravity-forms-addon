...
add_action( 'wp', array( 'TransactiumGFAddon', 'maybe_thankyou_page' ), 5 );
...
class TransactiumGFAddon extends GFPaymentAddOn {
	...
	public function init() {
		...
		add_action( 'gform_editor_js', array('TransactiumGFAddonHelpers', 'custom_attrib_editor_script') );
		add_action( 'gform_field_advanced_settings', array('TransactiumGFAddonHelpers', 'render_field_custom_attrib_setting') );
		
		add_filter( 'gform_notification_events', array('TransactiumGFAddonHelpers', 'notification_events'), 10, 2 );
		add_action( 'gform_post_payment_action', array('TransactiumGFAddonHelpers', 'send_notifications'), 10, 2 );
		...
		if ( !in_array( RG_CURRENT_PAGE, array( 'admin-ajax.php' ) ) ) {
			...
			switch ( RGForms::get( 'page' ) ) {
				...
				case 'gf_settings':
					...
					RGForms::add_settings_page( 'Transactium', array( 'TransactiumGFAddonHelpers', 'settings_page' ) );
					
					add_filter( 'gform_tooltips', array( 'TransactiumGFAddonHelpers', 'gform_tooltips' ) );
	...
	public function redirect_url( $feed, $submission_data, $form, $entry ) {
		...
		if (!TransactiumGFAddonHelpers::check_settings_complete($this->get_tr_settings()))
			return;
		...
		$request = array(
			'URLs' => array(
				'PushURL' => add_query_arg( array ( 'callback' => trim($this->_slug), 'reference' => urlencode($entry['id'] . '|' . wp_hash( $entry['id'] )) ), $this_url),
		...
		$entries['Billing_Email'] = $submission_data['email'];
		...
		foreach($form['fields'] as $field) {
			...
			if (!array_key_exists('fieldNamePOST', $field))
				continue;
			...
			if (!empty($value))
				$entries[$field['fieldNamePOST']] = trim($value);
		}
		...
		$soap = new SoapClient($hps_soap_url, array('trace' => 1,'cache_wsdl'=>'wsdl_cache_none'));
		$soap->__setSoapHeaders(new SOAPHeader($hps_ns,'HPSAuthHeader',array('Username'=>$hps_username,'Password'=>$hps_password)));
		$resp = $soap->CreateHostedPayment(array('Request'=>new SoapVar($request,SOAP_ENC_OBJECT,'HPPCreateRequest',$hps_ns)));
		//uncomment to DEBUG
		//error_log(json_encode($resp));
		return $resp->CreateHostedPaymentResult->URL;
	}
	...
	private function process_ipn( $config, $entry, $hps_resp, $hps_id ) {
		...
		switch ( strtolower( $status ) ) {
			...
			default:
				...
				if (!empty($hps_resp['GetHostedPaymentResult']['Transactions']) && !empty($hps_resp['GetHostedPaymentResult']['Transactions']['TransactionResponse']) && !empty($hps_resp['GetHostedPaymentResult']['Transactions']['TransactionResponse']['TransactionID']))
					$transaction_id = $hps_resp['GetHostedPaymentResult']['Transactions']['TransactionResponse']['TransactionID'];
				$formatted_amount = GFCommon::to_money( $amount, $entry['currency'] );
	...
	public function callback() {
		...
		$soap = new SoapClient($hps_url ,array('trace' => 1,'cache_wsdl'=>'wsdl_cache_none'));
		$soap->__setSoapHeaders(new SOAPHeader($hps_ns,'HPSAuthHeader',array('Username'=>$hps_username,'Password'=>$hps_password)));
		$hps_resp = $soap->GetHostedPayment(array("HPSID"=>$hps_id));
		//uncomment to DEBUG
		...
		if ( $str = rgget( 'transactium_gf_return' ) ) {
			...
			if ( wp_hash( 'ids=' . $query['ids'] ) == $query['hash'] ) {
				...
				if (empty($form['confirmations']))
					$form['confirmations'][$defaultConfirmationID] = $defaultConfirmationMessage;
				

				$confirmation = GFFormDisplay::handle_confirmation( $form, $lead, false, array( 'transaction_id', $lead['transaction_id'] ));
