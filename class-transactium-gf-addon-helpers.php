...
class TransactiumGFAddonHelpers {
	  ...
	  <script type="text/javascript">
		...
		<?php foreach($custom_attributes as $fieldRef => $fieldProps) { ?>
			...
			jQuery.map(fieldSettings, function (el, i) {
			  fieldSettings[i] += ', .<?php echo $fieldRef; ?>_setting';
			});
		 
			// Populate field settings on initialization.
			jQuery(document).on('gform_load_field_settings', function(ev, field){
			  var element = jQuery(document.getElementById('field_<?php echo $fieldRef; ?>')); 
			  if (element.attr('type') == "checkbox") {
				element.val(parseInt(field.<?php echo $fieldProps['fieldProperty']; ?>));
				element.prop("checked", field.<?php echo $fieldProps['fieldProperty']; ?> == "1" ? true : false);
			  } else if (element.attr('type') == "select" || element.localName == "select") {
				element.find("option[value='"+(field.<?php echo $fieldProps['fieldProperty']; ?> || '<?php echo $field_special_type_default; ?>')+"']").attr("selected","selected");
			  } else {
				element.val(field.<?php echo $fieldProps['fieldProperty']; ?> || '');
			  }
			});
	...
	?>
		...
		<?php if (!empty($custom_attributes_container_label)) { ?>
			<fieldset class="container" id="container" style="border-width:6px; padding: 1em;">
			  <legend>
			  <?php echo $custom_attributes_container_label; ?>
			  </legend>
		<?php } ?>
				<?php foreach($custom_attributes as $fieldRef => $fieldProps) { ?>
					<li class="<?php echo $fieldRef; ?>_setting field_setting" style="display: list-item;">
						<label for="field_<?php echo $fieldRef; ?>" class="section_label">
						  <?php echo esc_html_x( $fieldProps['gravitySettingLabel'], 'Label for Gravity Forms field '.$fieldProps['gravitySettingLabel'].' input' ); ?>
						</label>

						<?php if ($fieldProps['gravitySettingType'] == "checkbox") { ?>
							<input type="<?php echo $fieldProps['gravitySettingType']; ?>" id="field_<?php echo $fieldRef; ?>" onchange="jQuery(this).val(jQuery(this).prop('checked') == true ? 1 : 0); SetFieldProperty('<?php echo $fieldProps['fieldProperty']; ?>', this.value);"  />
						<?php } else if($fieldProps['gravitySettingType'] == "select") { ?>
							<select type="<?php echo $fieldProps['gravitySettingType']; ?>" id="field_<?php echo $fieldRef; ?>" onchange="SetFieldProperty('<?php echo $fieldProps['fieldProperty']; ?>', jQuery(this).val());">
								<?php foreach($fieldProps['gravitySettingValues'] as $value => $valueProps) { ?>
									<option value="<?php echo $value; ?>"><?php echo $value; ?></option>
								<?php } ?>
							</select>
						<?php } else { ?>
							<input type="<?php echo $fieldProps['gravitySettingType']; ?>" id="field_<?php echo $fieldRef; ?>" onchange="SetFieldProperty('<?php echo $fieldProps['fieldProperty']; ?>', this.value);" class="fieldwidth-3" size="35" />
						<?php } ?>
					</li>
				<?php } ?>
		<?php if (!empty($custom_attributes_container_label)) { ?>
	...
	public static function check_settings_complete($settings) {
		$all_fields = array("mode", "hps_username", "hps_password", "hps_tag");
		$required = array("mode", "hps_username", "hps_password", "hps_tag");
	...
	public static function get_merchant_request($entries) {
		...
		$editable_request = array(
			"Client" => array("ClientReference","OrderReference","MerchantUniqueReference","CardHolderName"),
			"Appearance" => array("ShopName","SkinFolder","LanguageCode","SiteBGColor"),
			"Billing" => array("FullName","Phone","Email","StreetNumber","StreetName","AddressUnitNumber","CityName","TerritoryCode","CountryCode","PostalCode","Fax","BirthDateYYYYMMDD"),
			"Customer" => array("FullName","Phone","Email","StreetNumber","StreetName","AddressUnitNumber","CityName","TerritoryCode","CountryCode","PostalCode","Fax","BusinessName","BusinessRegistrationNumber","BusinessTaxNumber"),
			"Shipping" => array("FullName","Phone","Email","StreetNumber","StreetName","AddressUnitNumber","CityName","TerritoryCode","CountryCode","PostalCode","Fax","BusinessName")
		);
		...
		foreach($entries as $postName => $value) {
			...
			foreach($postNameEntries as $postNameEntry)
			{
				...
				if (sizeof($varTree) == 2 && in_array($varTree[1], $editable_request[$varTree[0]])) {
					if (!isset($request[$varTree[0]]))
						$request[$varTree[0]] = array();
